...
module.exports = {
  name: "config",
  description: "Edit pengaturan bot",
  usage: "",
  permissions: {
...
`);
    ...
    let emoji = await ConfigMessage.awaitReactions(
      ...
      client.sendTime(
        message.channel,
        "❌ | **Anda terlalu lama menanggapi. Jika Anda ingin mengedit pengaturan, jalankan perintah lagi!**"
      );
    ...
    if (em._emoji.name === "1️⃣") {
      await client.sendTime(
        message.channel,
        "Apa yang ingin Anda ubah awalannya?"
      );
      ...
      await client.sendTime(
        message.channel,
        "Sebutkan peran yang Anda inginkan `DJ's` memiliki."
      );
      ...
      if (!role.mentions.roles.first())
        return client.sendTime(
          message.channel,
          "Sebutkan peran yang Anda inginkan Hanya Untuk DJ's Saja."
        );
      ...
      client.sendTime(
        message.channel,
        "Berhasil disimpan DJ berperan sebagai <@&" + role.id + ">"
      );
    }
  },
  ...
  SlashCommand: {
    options: [
      {
        name: "prefix",
        description: "Periksa awalan bot",
        type: 1,
        required: false,
        options: [
          {
            name: "symbol",
            description: "Tetapkan awalan bot",
            type: 3,
            required: false,
          },
        ],
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (config === "prefix") {
        ...
        if (
          ...
          client.sendTime(
            interaction,
            `Awalan sekarang telah diatur ke \`${prefix}\``
          );
        } else {
          //has not prefix
          client.sendTime(
            interaction,
            `Awalan dari server ini adalah \`${GuildDB.prefix}\``
          );
        }
