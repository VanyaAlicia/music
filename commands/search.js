...
module.exports = {
  name: "search",
  description: "Menampilkan hasil lagu berdasarkan permintaan pencarian",
  usage: "[song]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (Searched.loadType == "NO_MATCHES")
      ...
      let msg = await message.channel.send(
        "**Ketikkan nomor lagu yang ingin Anda mainkan! Berakhir dalam `30 Detik`.**"
      );
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "**Anda terlalu lama menanggapi. Jalankan perintah lagi jika Anda ingin memainkan sesuatu!**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(
          message.channel,
          "Silakan kirim nomor ID lagu yang benar"
        );
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song)
        return client.sendTime(
          message.channel,
          "Tidak ada lagu yang ditemukan untuk ID yang diberikan"
        );
    ...
    options: [
      {
        ...
        type: 3,
        required: true,
        description: "Masukkan nama lagu atau url yang ingin dicari",
      },
    ],
    /**
     *
        ...
        switch (res.loadType) {
          ...
          case "SEARCH_RESULT":
            ...
            const resultss = new MessageEmbed()
              .setDescription(
                `${results}\n\n\t**Ketikkan nomor lagu yang ingin Anda mainkan!**\n`
              )
              .setColor(client.botconfig.EmbedColor)
              .setAuthor(
            ...
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `Jumlah yang Anda berikan lebih besar atau lebih kecil dari total pencarian. Usage - \`(1-${max})\``
              );
